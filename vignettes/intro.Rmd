...
```{r}
h_load_data <- hnode('NA -> a')
h_normalize <- hnode('a -> b')
h_dist      <- hnode('b -> c')
h_hclust    <- hnode('c -> d')
h_hclust()
```
...
```{r}
h_load_data <- hnode('NA -> a', f=f_load_data)
h_normalize <- hnode('a -> b',  f=f_normalize, inode=h_load_data)
h_dist      <- hnode('b -> c',  f=f_dist,      inode=h_normalize)
h_hclust    <- hnode('c -> d',  f=f_hclust,    inode=h_dist)
h_hclust()
```
